...
# Step 1: Read the data from the spreadsheet using pandas
def read_data():
    df = pd.read_csv('data/coffee.csv', encoding='latin-1')  # Adjust encoding if needed
    return df
...
# Step 4: Count how many coffees from each country
...
def plot_countries_num_coffees(country_counted):
    ...
    plt.subplots_adjust(left=0.2, right=0.9)  # play around with margins
    plt.tight_layout()  # makes layout more compact, all country names fitted

    plt.show(block=False)  # Display the plot


# Step 6: Calculate the average rating of coffee for each country
...
# Step 7: Calculate correlation between Prices of all coffees and rating and IF statements for significance
...
# Step 7: Plot graph of price vs ratings
    ...
    plt.xlabel('Price (USD) of Coffee per 100g (log)')
...
# Step 8: Ask user which roast type and generate word map with reviews from that roast type
...
def generate_roast_word_cloud():
    ...
    # Prompt if invalid roast type entered
    ...
    # Determine the column name based on the roast type selected
    review_data = f"review"  # Assuming column names follow a pattern like "Light_Review"

    # gather review word data from the selected column
    ...
    stopwords.update(["aroma", "note", "cup", "flavor", "flavors", "hint", "toned", "structure",
                      "notes", "cacao", "cacao nib", "cocoa nib", "nib"]
                     )
